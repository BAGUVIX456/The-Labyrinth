// Determines whether specific things can be placed in specific places
...
{
    // Set of all tiles where items can be placed separated depending on whether it is open space or near a wall
    ...
    public ItemPlacementHelper(HashSet<Vector2Int> roomFloor, HashSet<Vector2Int> roomFloorNoCorridor)
    {
        ...
        foreach (var position in roomFloorNoCorridor)
        {
            ...
            // Play around with this statement
            if (type == PlacementType.NearWall && graph.GetNeighbours4Directions(position).Count < 2)
                continue;
            tileByType[type].Add(position);
        }
    }

    public Vector2Int GetItemPlacementPosition(PlacementType placementType, int iterationsMax, Vector2Int size, bool addOffset)
    {
        int itemArea = size.x * size.y;
        
        // If area of free space is less than itemArea then return nothing
        ...
        for(int iteration = 0; iteration < iterationsMax; iteration++)
        {
            ...
            // Not needed for the near future
            if (itemArea > 1)
            {
